...
namespace Cat5201
{
    ...
    public sealed class GoogleDriveService
    {
        ...
        public const string GoogleSlidesMime = "application/vnd.google-apps.presentation";

        /// <summary>
        /// P3-3：Google 原生格式 → 匯出計畫（匯出 mime / 存檔副檔名 / 給使用者看的說明）。
        /// 選 LLM 友善的純文字類格式：Docs/Slides→txt、Sheets→csv（CSV 只含第一張工作表）。
        /// 不支援的原生格式（繪圖/表單等）回 null。
        /// </summary>
        public static (string ExportMime, string Ext, string Label)? GetExportPlan(string? googleMime) => googleMime switch
        {
            GoogleDocMime => ("text/plain", ".txt", "轉為文字"),
            GoogleSheetMime => ("text/csv", ".csv", "轉為 CSV·第一張工作表"),
            GoogleSlidesMime => ("text/plain", ".txt", "轉為文字"),
            _ => null,
        };

        /// <summary>匯出 Google 原生檔案（Docs/Sheets/Slides）成一般格式到本機（附件輸入用）。</summary>
        public async Task ExportFileAsync(string fileId, string exportMime, string savePath, CancellationToken ct)
        {
            string token = await EnsureAccessTokenAsync(ct).ConfigureAwait(false);
            using var req = new HttpRequestMessage(
                HttpMethod.Get,
                $"https://www.googleapis.com/drive/v3/files/{Uri.EscapeDataString(fileId)}/export?mimeType={Uri.EscapeDataString(exportMime)}");
            req.Headers.Authorization = new AuthenticationHeaderValue("Bearer", token);
            ...
            {
                string body = await resp.Content.ReadAsStringAsync(ct).ConfigureAwait(false);
                throw new InvalidOperationException($"Drive 匯出失敗（{(int)resp.StatusCode}）：{body}");
            }
