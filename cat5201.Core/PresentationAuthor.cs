...
要求：
- {slideCountRule}
- 每張投影片依訊息選 1～4 個重點，不必每頁相同數量；每點約 18～40 個中文字，標題約 12～24 字，讓畫面能使用大字與留白。數據頁保留完整正負號、單位、時間，優先一點一個指標。
- 畫面只放觀眾需要看的內容；不要把「主視覺建議」「配圖建議」「此頁可使用」「設計方向」等製作指令寫進標題或重點。避免每頁都是三段文章。
...
要求：
- 1～4 個重點，每點約 18～40 個中文字，保留必要的數字、完整單位與時間；不要把配圖、版面或製作建議寫進觀眾看到的內容。
- 用與【簡報主題】相同的語言。
- 只給這一張的內容，不要輸出其它張。";
        }
